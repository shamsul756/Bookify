...
const BookDetails = () => {
    ...
    if (!expectedBook) {
        return <p className="text-center py-20 text-base-content/50">Book not found.</p>;
    }

    return (
        <div className="max-w-3xl mx-auto my-8 bg-base-300 border border-base-200 rounded-2xl overflow-hidden flex flex-col lg:flex-row">
            ...
            <div className="flex-1 p-6 lg:p-7 flex flex-col gap-0 min-w-0">
                ...
                <div className="flex gap-2 flex-wrap mb-3">
                    {expectedBook.tags.map((tag, i) => (
                        <span key={i} className="text-xs font-medium px-3 py-1 rounded-full bg-green-50 text-green-700 border border-green-300">
                            {tag}
                        </span>
                    ))}
                </div>
                ...
                <div className="flex gap-3 mt-auto">
                    <button onClick={() => handleMarkasRead(expectedBook)} className="btn btn-outline flex-1">Mark as Read</button>
                    <button onClick={()=> handleWishList(expectedBook) } className="btn flex-1 bg-green-600 hover:bg-green-700 text-white border-none">
                        Add to Wishlist
                    </button>
                </div>
